...
RSpec.describe Mobilis::Project::MetaProject do
  let(:metaproject) { build(:metaproject) }
  it "factory works" do
    expect(metaproject.instance_of?(Mobilis::Project::MetaProject)).to be_truthy
  end
  ...
  describe "#to_h" do
    ...
    it "works for simple metaproject case" do
      expected = { projects: [], username: "meleneth", starting_port_no: 10_000, port_gap: 100, name: "meta_project" }
      expect(metaproject.to_h).to eq(expected)
    end
    it "works for simple rails project case" do
      expected = { name: "my_prime", type: :rails, controllers: [], models: [], options: options, attributes: {},
                   links: [] }
      expect(rails_project.to_h).to eq(expected)
    end
    it "works for more complicated case" do
      expected = {
        projects: [{ name: "my_prime", type: :rails, controllers: [], models: [], options: options, attributes: {},
                     links: [] }], username: "meleneth", starting_port_no: 10_000, port_gap: 100, name: "meta_project"
      }
...
RSpec.describe Mobilis::Project::RailsProject do
  let(:rails_project) { build(:rails_prime, name: "my_prime") }
  it "factory works" do
    expect(rails_project.instance_of?(Mobilis::Project::RailsProject)).to be_truthy
