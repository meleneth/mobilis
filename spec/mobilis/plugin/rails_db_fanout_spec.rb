...
RSpec.describe Mobilis::Plugin::RailsDBFanout do
  ...
  describe "initialization" do
    ...
    it "sets CACHE_DATABASE_URL on the rails container" do
      ...
      expect(realized_rails_node.env_var("CACHE_DATABASE_URL").specific_name).to eq("USERDB_CACHE_DATABASE_URL")
    end
    it "sets CABLE_DATABASE_URL on the rails container" do
      plugin.hook_envs_realized
      realized_rails_node = manifest.realized_env(:production).node_by_name("user")
      expect(realized_rails_node.env_var("CABLE_DATABASE_URL").specific_name).to eq("USERDB_CABLE_DATABASE_URL")
    end
    it "sets QUEUE_DATABASE_URL on the rails container" do
      plugin.hook_envs_realized
      realized_rails_node = manifest.realized_env(:production).node_by_name("user")
      expect(realized_rails_node.env_var("QUEUE_DATABASE_URL").specific_name).to eq("USERDB_QUEUE_DATABASE_URL")
    end
  end
end
